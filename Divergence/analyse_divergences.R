...
div = read.table("ALL_DIVERGENCES.tsv",sep="\t",header=TRUE)
...
par(mar=c(10,4,4,4))
boxplot(identity~factor,data=div_,main=paste(qsp,"vs",tsp,"by origin=",origin),xlab="",ylab="%identity",las=2,cex.names=.6,cex.main=.6,varwidth=TRUE)
...
div_$factor = paste(gsub("SUPER_","",div_$chr)," vs ",gsub("SUPER_","",div_$chr2),sep="")
par(mar=c(10,4,4,4))
boxplot(identity~factor,data=div_,main=paste(qsp,"vs",tsp,"by origin=",origin),xlab="",ylab="%identity",las=2,cex.names=.6,cex.main=.6,varwidth=TRUE,ylim=c(0.4,1))
